...
form.addEventListener('submit', function (e) {
    ...
    */
    ...
    fields.forEach(function (field) {
        ...
        const fieldValidation = field.validator(inputValue);
        if (!fieldValidation.isValid) {
            errorSpan.innerHTML = `${errorIcon} ${fieldValidation.errorMessage}`;
    ...
    function nameIsValid(value) {
        ...
        if (value.length < min) {
            validator.isValid = false;
            validator.errorMessage = `O campo deve conter pelo menos ${min} caracteres.`;
            return validator;
        }
...
function validateField(input, validator) {
    ...
    if (!validation.isValid) {
        errorSpan.innerHTML = `${errorIcon} ${validation.errorMessage}`;
        inputBox.classList.add('invalid');
    } else {
        inputBox.classList.add('valid');
    }
...
passwordInput.addEventListener('input', function () {
    validateField(passwordInput, fieldIsRequired);
    validateField(confirmPasswordInput, passwordMatch);
});
